...
export class PollsRepository {
  ...
  async createPoll({
    ...
    this.logger.log(
      `Creating new poll: ${JSON.stringify(initialPoll, null, 2)} whith TTL ${
        this.ttl
      }`,
    );
  ...
  async addNomination({
    ...
    this.logger.log(
      `Attemptiing yo add a nomination with nominationID/nomination: 
  ...
  async removeNomination(pollID: string, nominationID: string): Promise<Poll> {
    this.logger.log(
      `removing nominztionID: ${nominationID} from poll: ${pollID}`,
    );
  ...
  async startPoll(pollID: string): Promise<Poll> {
    ...
    try {
      ...
      this.logger.error(`Failed set hasStarted for poll: ${pollID}`);
      throw new InternalServerErrorException(
  ...
  async addParticipantRankings({
    ...
    try {
      ...
      this.logger.error(
        `Failed to add ankings for userID/name: ${userID} to poll: ${pollID}`,
        rankings,
      );
      throw new InternalServerErrorException(
